...
import {
  ...
  const ProfileTopSection = () => {
    ...
    const name = "John Doe"; // Replace with actual name from state
    const username = "johndoe"; // Replace with actual username from state
    const joinedAt = "March 2023"; // Replace with actual joined date
    ...
    return (
      <section className="flex flex-row-reverse border-b-2 border-gray-200 pb-8 md:flex-row md:gap-8">
        ...
        <div className="flex grow flex-col justify-between gap-3">
          ...
          {/* Replace with actual Flag component */}
          <div>{language}</div>
        </div>
        <a
  ...
  const ProfileStatsSection = () => {
    const streak = 5; // Replace with actual streak from state
    const totalXp = 125;
    const league = "Bronze";
  ...
  const BottomBar = ({ selectedTab }) => {
    // Add your bottom bar component or replace it
    return <div>Bottom Bar - {selectedTab}</div>;
  };
  
  const LeftBar = ({ selectedTab }) => {
    // Add your left bar component or replace it
    return <div>Left Bar - {selectedTab}</div>;
  };